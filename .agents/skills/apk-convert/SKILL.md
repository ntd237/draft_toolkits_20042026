...
**Sub-skills you orchestrate (all under `.zcode/skills/`):**
...
- **Signing & package:** Ask once at the start, reuse the answer for the converter.
...
3. Resolve options (ask once, defaults in parentheses):
   - `direction` — `free2paid` | `paid2free` | `auto` (auto = infer from analyzer guess)
   - `signing` — `reuse original cert` | `provide keystore` | `create debug keystore` (default: debug keystore)
   - `package` — `keep` | `change to <orig>.paid/.free` (default: keep; ask during convert if not decided)
4. Record resolved options for downstream skills.
...
3. The converter must: backup `work/convert/original.apk`, patch (ads + premium gates), rebuild, zipalign, and emit BOTH `dist/app-*-debug-unsigned.apk` and `dist/app-*-release-signed.apk` plus `PATCH_REPORT.md`.
...
2. It must: install via `adb` (or boot an AVD if no device), run S1 no-crash + S2 navigation + S3 edition check (ads hidden for paid, visible for free), capture `logcat.txt`.
...
1. Print a summary table:
   - Input, detected edition, chosen direction, package decision, signing choice
...
2. List every artifact path. Never claim success without `VERIFY_REPORT.md` showing `PASSED`.
...
work/convert/PATCH_REPORT.md
work/convert/original.apk        # backup
...
- Preserve `work/convert/original.apk` untouched; sub-skills restore from it.
...
- Always end with `VERIFY_REPORT.md`; success = `PASSED` with `pidof` alive + zero FATAL + edition check passed.
...
- [ ] `VERIFY_REPORT.md` shows `PASSED` with S1/S2/S3 evidence and `logcat.txt` attached?
- [ ] Summary table lists absolute paths for all deliverables?
...
# Explicit direction, keep package, debug keystore
/apk-convert file: app-paid.xapk direction: paid2free signing: debug package: keep
