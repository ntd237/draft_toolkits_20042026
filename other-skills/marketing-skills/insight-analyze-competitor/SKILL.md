...
- **Quy trình thu thập dữ liệu 3 tầng (3-Tier Data Ingestion Strategy)** tuân thủ `config.md`:
  ...
  - **Tầng 2 (Browser Automation — Browser Subagent First, MCP Fallback)**:
    - **Ưu tiên 1 (Browser Subagent của Antigravity IDE)**: Luôn ưu tiên sử dụng Browser Subagent (`browser` subagent) của Antigravity IDE thông qua `Browser Settings` (yêu cầu cài đặt Google Chrome và bật `Enable Browser Tools`). Subagent này sử dụng trình duyệt Chrome thật trên máy để tải trang web động (SPA), tương tác trực quan và dễ dàng vượt qua các bước xác thực người dùng.
    - **Ưu tiên 2 (Playwright MCP Fallback)**: Nếu Browser Subagent chưa được bật hoặc cần chạy tự động ngầm trong session hiện tại, chuyển sang sử dụng MCP server `playwright` (`browser_navigate`, `browser_snapshot`, `browser_take_screenshot`) để trích xuất cấu trúc accessibility tree và chụp ảnh màn hình lưu bằng chứng.
  - **Tầng 3 (Human-in-the-Loop Fallback khi cả 2 bị chặn hoàn toàn)**:
    - Các nền tảng như **Meta Ads Library**, **Google Ads Transparency**, **TikTok** có thể áp dụng rào cản bảo mật cao (Cloudflare Turnstile, CAPTCHA phức tạp, yêu cầu đăng nhập tài khoản).
    - **Khi cả 2 công cụ browser trên đều bị chặn, Agent KHÔNG dừng lại báo lỗi cụt hoặc tự bịa đặt thông tin**, mà **PHẢI** giải thích ngắn gọn nguyên nhân kỹ thuật và hướng dẫn người dùng cung cấp dữ liệu qua:
      1. *Dán ảnh màn hình (Multimodal Vision)*: Người dùng chụp 2-3 mẫu quảng cáo đối thủ từ Meta Ads Library rồi dán (paste) trực tiếp vào khung chat. Agent tận dụng khả năng xử lý ảnh để bóc tách Hook, Angle, Visual và CTA.
      2. *Cung cấp link Landing page*: Gửi link trang đích bán hàng của quảng cáo (thay vì link thư viện ads).
      3. *Copy-paste text*: Dán trực tiếp các mẫu headline, hook, offer của đối thủ vào chat.
